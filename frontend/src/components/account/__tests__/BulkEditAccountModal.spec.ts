...
describe('BulkEditAccountModal', () => {
  ...
  it('OpenAI OAuth 批量关闭指纹收敛时显式提交 off', async () => {
    const wrapper = mountModal({
      selectedPlatforms: ['openai'],
      selectedTypes: ['oauth']
    })

    await wrapper.get('[data-testid="bulk-codex-fingerprint-enabled"]').setValue(true)
  ...
  it('OpenAI OAuth 批量编辑选择「关闭」时应显式提交 codex_fingerprint_mode=off（issue #6327）', async () => {
    ...
    // 二开默认是账号级平衡，因此先显式选择 off，再验证批量接口不会把关闭操作省略掉。
