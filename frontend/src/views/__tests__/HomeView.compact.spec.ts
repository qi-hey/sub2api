    ...
    global: {
      stubs: {
        ...
        DynamicHomeHero: {
          ...
          props: { showModelPlazaEntry: Boolean },
          template: '<div data-testid="dynamic-home" />',
        },
      },
    },
  })
...
describe('HomeView compact mode', () => {
  ...
  it.each([undefined, false])('selects the dynamic home when compact mode is %s', (enabled) => {
    const settings = enabled === undefined ? {} : { compact_home_enabled: enabled }
    const wrapper = mountHome(settings)

    expect(wrapper.find('[data-testid="compact-home"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="dynamic-home"]').exists()).toBe(true)
  })
  ...
  it('shows the model plaza link in the default home header', () => {
    ...
    expect(wrapper.getComponent({ name: 'DynamicHomeHero' }).props('showModelPlazaEntry')).toBe(true)
  })
