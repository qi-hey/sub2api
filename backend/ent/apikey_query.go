...
func (_q *APIKeyQuery) QueryGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *APIKeyQuery) WithGroups(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withGroups; query != nil {
		if err := _q.loadGroups(ctx, query, nodes,
			func(n *APIKey) { n.Edges.Groups = []*Group{} },
			func(n *APIKey, e *Group) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.GroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(apikey.GroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(apikey.GroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.GroupsPrimaryKey[0]))
