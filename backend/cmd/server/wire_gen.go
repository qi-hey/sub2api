...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	gameWalletRepository := repository.NewGameWalletRepository(db)
	_ = gameWalletRepository // R20 exchange repo retained for rollback compatibility.
	gameLoyaltyRepository := repository.NewGameLoyaltyRepository(db)
	gameLoyaltyService := service.NewGameLoyaltyService(gameLoyaltyRepository, settingService, apiKeyAuthCacheInvalidator, billingCache)
	gameWalletHandler := handler.NewGameWalletHandler(gameLoyaltyService)
	...
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, subscriptionService, configConfig, coordinator)
	...
	modelPlazaHandler := handler.NewModelPlazaHandler(channelService, apiKeyService, settingService, gatewayService)
