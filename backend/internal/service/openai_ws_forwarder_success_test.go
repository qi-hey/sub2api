...
func TestOpenAIGatewayService_BuildOpenAIWSHeadersLegacyDeviceModeUsesBalancedIdentity(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	require.Equal(t, wantThread, gjson.Get(payloadJSON, "prompt_cache_key").String())
