...
func resolveOpenAIAccountUpstreamModelForRequest(account *Account, requestedModel string, requireCompact bool) string {
	...
	// OAuth passthrough only replaces authentication. Custom API-key passthrough
	// additionally applies the account model_mapping before forwarding, which is
	// required by compatible upstream pools such as Any Router.
	if account != nil && account.IsOpenAIPassthroughEnabled() {
		...
		if account.Type == AccountTypeAPIKey {
			upstreamModel = resolveOpenAIForwardModel(account, upstreamModel, "")
			if upstreamModel == "" {
				return ""
			}
		}
