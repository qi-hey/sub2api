...
func TestOpenAIGatewayService_OAuthPassthrough_CompactUsesJSONAndKeepsNonStreaming(t *testing.T) {
	...
	c.Request.Header.Set("User-Agent", "codex_cli_rs/0.1.0")
	c.Request.Header.Set("Content-Type", "application/json")
	c.Request.Header.Set("session-id", "compact-client-session")
	...
	wantSession := resolveConvergedSessionID(testCodexFingerprintSeed)
	wantThread := resolveConvergedThreadID(testCodexFingerprintSeed, "compact-client-session")
...
func TestOpenAIGatewayService_OpenAIPassthrough_NonStreamDetectsMislabeledSSE(t *testing.T) {
	...
	result, err := svc.handleNonStreamingResponsePassthrough(context.Background(), resp, c, nil, "gpt-5.5", "gpt-5.5")
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestOpenAIGatewayService_OpenAIPassthrough_APIKeyCodexHeadersAreCompleted(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid-relay-headers"}},
...
func TestOpenAIGatewayService_OpenAIPassthrough_APIKeyCodexReasoningIncludeCompleted(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid-relay-include"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_1","model":"gpt-5.5","output":[],"usage":{"input_tokens":1,"output_tokens":1}}`)),
	}}
	svc := &OpenAIGatewayService{
		cfg: &config.Config{Security: config.SecurityConfig{URLAllowlist: config.URLAllowlistConfig{
...
func TestOpenAIGatewayService_OAuthPassthrough_OfficialIdentityUnified(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(nil))
	c.Request.Header.Set("User-Agent", "codex_vscode/0.140.2 (Mac OS X 14.0; arm64) vscode (codex_vscode; 0.140.2)")
	...
	account := &Account{
		ID: 123, Name: "acc", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
...
func TestOpenAIGatewayService_CodexFingerprintCompactKeepsBodyAndConvergesHeaders(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIGatewayService_APIKeyPassthrough_AppliesAccountMappingAndUsesResponsesEndpoint(t *testing.T) {
	gin.SetMode(gin.TestMode)
