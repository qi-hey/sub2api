...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if compactRequest {
		compactMappedModel := s.resolveOpenAICompactFallbackModel(account, passthroughForwardModel)
	...
	if account != nil && account.UsesOpenAICodexProtocol() {
		...
		// 指纹收敛：与非透传路径同门控。Compact 请求体保持原样，
		// 但出站头仍应用账号级指纹，避免同一账号暴露第二套身份。
		...
		if clientRequestedStream {
			...
			if handleErr != nil {
				...
				if signal, ok := asOpenAICompactFallbackSignal(handleErr); ok {
					...
					if shouldFailoverOpenAIPassthroughResponse(c, account, compactResp.StatusCode, compactBody) {
