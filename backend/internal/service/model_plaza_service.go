...
// ListGroupsWithModelFallback augments the channel-backed plaza with models
// exposed by account-pool groups. Channel pricing remains authoritative when
// both sources expose the same model.
func (s *ModelPlazaService) ListGroupsWithModelFallback(
	ctx context.Context,
	fallback func(*Group) []string,
) ([]PlazaGroup, error) {
	return s.listGroups(ctx, fallback)
}

func (s *ModelPlazaService) listGroups(
	ctx context.Context,
	fallback func(*Group) []string,
) ([]PlazaGroup, error) {
	...
	if fallback != nil {
		for i := range groups {
			...
			for _, modelName := range fallback(g) {
				...
				key := modelKey{platform: g.Platform, name: modelName}
...
func (s *ModelPlazaService) lookupAccountPoolPricing(modelName string) *ChannelModelPricing {
