...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	return r.withAPIKeyTx(ctx, func(txCtx context.Context, client *dbent.Client) error {
...
func (r *apiKeyRepository) createWithClient(ctx context.Context, client *dbent.Client, key *service.APIKey) error {
	groupIDs, err := normalizedAPIKeyGroupIDs(key.GroupID, key.GroupIDs)
	if err != nil {
		return err
	}
	...
	if err := replaceAPIKeyGroupBindings(ctx, client, created.ID, groupIDs); err != nil {
		return err
	}
	key.ID = created.ID
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey) error {
	...
	now := time.Now()
	err = r.withAPIKeyTx(ctx, func(txCtx context.Context, client *dbent.Client) error {
		...
		return replaceAPIKeyGroupBindings(txCtx, client, key.ID, groupIDs)
	})
	if err != nil {
		return err
	}
...
func normalizedAPIKeyGroupIDs(defaultGroupID *int64, groupIDs []int64) ([]int64, error) {
	if len(groupIDs) == 0 {
		if defaultGroupID == nil {
			return nil, nil
		}
		return []int64{*defaultGroupID}, nil
	}
	if defaultGroupID == nil {
		return nil, service.ErrAPIKeyGroupNotBound
	}
...
func (r *apiKeyRepository) apiKeyListByUserIDQuery(userID int64, filters service.APIKeyListFilters) *dbent.APIKeyQuery {
	...
	if filters.GroupID != nil {
		if *filters.GroupID == 0 {
			q = q.Where(
				apikey.GroupIDIsNil(),
				apikey.Not(apikey.HasGroups()),
			)
		} else {
			q = q.Where(apikey.Or(
				apikey.GroupIDEQ(*filters.GroupID),
				apikey.HasGroupsWith(group.IDEQ(*filters.GroupID)),
			))
		}
	}
...
func (r *apiKeyRepository) ListByGroupID(ctx context.Context, groupID int64, params pagination.PaginationParams) ([]service.APIKey, *pagination.PaginationResult, error) {
	q := r.activeQuery().Where(apikey.Or(
		...
		apikey.HasGroupsWith(group.IDEQ(groupID)),
	))
...
// ClearGroupIDByGroupID removes one group binding from all active API keys.
// Keys that used it as their default move to the smallest remaining binding.
func (r *apiKeyRepository) ClearGroupIDByGroupID(ctx context.Context, groupID int64) (int64, error) {
	...
	err := r.withAPIKeyTx(ctx, func(txCtx context.Context, client *dbent.Client) error {
		keys, err := client.APIKey.Query().
			...
			Order(dbent.Asc(apikey.FieldID)).
			Select(apikey.FieldID, apikey.FieldGroupID).
			All(txCtx)
		if err != nil {
			return err
		}
		...
		apiKeyIDs := make([]int64, 0, len(keys))
		defaultKeyIDs := make([]int64, 0, len(keys))
		for _, key := range keys {
			apiKeyIDs = append(apiKeyIDs, key.ID)
			if key.GroupID != nil && *key.GroupID == groupID {
				defaultKeyIDs = append(defaultKeyIDs, key.ID)
			}
		}

		if _, err := client.APIKeyGroup.Delete().
			Where(
				apikeygroup.APIKeyIDIn(apiKeyIDs...),
		...
		if len(defaultKeyIDs) > 0 {
			remaining, err := client.APIKeyGroup.Query().
				Where(apikeygroup.APIKeyIDIn(defaultKeyIDs...)).
				Order(
					dbent.Asc(apikeygroup.FieldAPIKeyID),
					dbent.Asc(apikeygroup.FieldGroupID),
				).
				All(txCtx)
			if err != nil {
				return err
			...
			nextDefaultByKeyID := make(map[int64]int64, len(defaultKeyIDs))
			for _, binding := range remaining {
				if _, exists := nextDefaultByKeyID[binding.APIKeyID]; !exists {
					nextDefaultByKeyID[binding.APIKeyID] = binding.GroupID
				}
			}
			for _, apiKeyID := range defaultKeyIDs {
				builder := client.APIKey.UpdateOneID(apiKeyID).
					Where(apikey.DeletedAtIsNil())
				if nextDefault, ok := nextDefaultByKeyID[apiKeyID]; ok {
					builder.SetGroupID(nextDefault)
				} else {
					builder.ClearGroupID()
				}
...
// UpdateGroupIDByUserAndGroup migrates one binding for all active keys owned by a user.
// Other bindings are preserved, and a legacy default is moved only when it matched oldGroupID.
func (r *apiKeyRepository) UpdateGroupIDByUserAndGroup(ctx context.Context, userID, oldGroupID, newGroupID int64) (int64, error) {
	...
	err := r.withAPIKeyTx(ctx, func(txCtx context.Context, client *dbent.Client) error {
		...
		builders := make([]*dbent.APIKeyGroupCreate, 0, len(apiKeyIDs))
		...
		if err := client.APIKeyGroup.CreateBulk(builders...).
			OnConflictColumns(apikeygroup.FieldAPIKeyID, apikeygroup.FieldGroupID).
			DoNothing().
			Exec(txCtx); err != nil {
			return err
		}
		...
		if len(defaultKeyIDs) > 0 {
			if _, err := client.APIKey.Update().
				Where(apikey.IDIn(defaultKeyIDs...), apikey.DeletedAtIsNil()).
				SetGroupID(newGroupID).
				Save(txCtx); err != nil {
				return err
			}
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	groupsByID := make(map[int64]service.Group, len(m.Edges.Groups))
	for _, entity := range m.Edges.Groups {
		converted := groupEntityToService(entity)
		if converted == nil {
			continue
		}
		if _, exists := groupsByID[converted.ID]; exists {
			continue
		}
		groupsByID[converted.ID] = *converted
	}
	if out.Group != nil {
		if _, exists := groupsByID[out.Group.ID]; !exists {
			groupsByID[out.Group.ID] = *out.Group
		}
	}
	if len(groupsByID) > 0 {
		out.GroupIDs = make([]int64, 0, len(groupsByID))
		...
		out.Groups = make([]service.Group, 0, len(out.GroupIDs))
		for _, groupID := range out.GroupIDs {
			out.Groups = append(out.Groups, groupsByID[groupID])
		}
	}
	return out
}
